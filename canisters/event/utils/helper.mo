...
module {
  ...
  public func getMiscAttributeDataValueArray(array : [Principal]) : [Database.MiscAttributeDataValue] {
    let initialBuffer = Buffer.fromArray<(Database.MiscAttributeDataValue)>([]);
    let valuesToBeAppended = Buffer.Buffer<(Database.MiscAttributeDataValue)>(0);

    for (element in array.vals()) {
      valuesToBeAppended.add(#principal(element));
    };
